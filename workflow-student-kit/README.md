...
```text
$workflow
請啟動或接續目前專案；新流程先以 $grill-with-docs 確認意圖與需求，續作直接接續。
```

新流程以 `$grill-with-docs` 開始，每次釐清一個未決問題，確認目標、範圍、
驗收條件與停止條件後，才進入專案設定及後續交付流程。
續作不必重新確認；只有使用者提出新增需求時，才重新進入釐清，並沿用已確認的決策。
僅查詢狀態或切換模式時，直接完成該操作。
